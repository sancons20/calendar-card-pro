...
The `start_date` parameter now supports relative date expressions, allowing your calendar to display events from a consistent time offset rather than a fixed date.

**Formats supported:**

- `today+n`: Show events starting n days in the future
- `today-n`: Show events starting n days in the past
- `+n`: Shorthand for today+n
- `-n`: Shorthand for today-n

**Examples:**

- `start_date: "today+7"`: Always show events starting next week
- `start_date: "+3"`: Always show events starting 3 days from today
- `start_date: "today-2"`: Always show events starting from 2 days ago

This feature is especially useful for dashboards where you want to maintain a consistent view relative to the current date, such as "always show next week's events" instead of a fixed date range.
...
| start_date                                 | string  | Today                             | Custom start date for the calendar (YYYY-MM-DD format). Also supports relative date expressions like `today+7` to show events starting 7 days from today, or `today-3` to show events from 3 days ago. You can also use shorthand notation `+7` or `-3`. |
